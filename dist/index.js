...
app.get('/api/manhwas', async (c) => {
    try {
        ...
        if (q) {
            conditions.push(or(ilike(manhwas.title, `%${q}%`), ilike(manhwas.alternativeTitles, `%${q}%`)));
        }
        if (type) {
        ...
        let orderBy = desc(manhwas.createdAt);
        if (sort === 'oldest')
            orderBy = asc(manhwas.createdAt);
        if (sort === 'rating')
            orderBy = desc(manhwas.rating);
        if (sort === 'title')
            orderBy = asc(manhwas.title);
...
async function resolveTags(items) {
    const ids = [];
    for (const item of items) {
        if (typeof item === 'number') {
            ids.push(item);
...
app.post('/api/admin/manhwas', checkRole(['admin', 'moderator']), async (c) => {
    try {
        ...
        const { categories: categoryItems, tags: tagItems, characters: characterItems, staff: staffItems, id, createdAt, updatedAt, ...insertData } = data;
        ...
        if (categoryItems && Array.isArray(categoryItems)) {
            ...
            const catLinks = categoryIds.map((catId) => ({ manhwaId, categoryId: catId }));
        ...
        if (tagItems && Array.isArray(tagItems)) {
            ...
            const tagLinks = tagIds.map((tId) => ({ manhwaId, tagId: tId }));
        ...
        if (characterItems && Array.isArray(characterItems)) {
            const charValues = characterItems.map((char) => ({
                manhwaId,
                name: char.name,
                image: char.image || '',
                role: char.role || 'MAIN',
...
app.put('/api/admin/manhwas/:id', checkRole(['admin', 'moderator']), async (c) => {
    try {
        ...
        const { categories: categoryItems, tags: tagItems, characters: characterItems, staff: staffItems, id: _, createdAt, updatedAt, ...updateData } = data;
        ...
        if (categoryItems && Array.isArray(categoryItems)) {
            ...
            const categoryIds = await resolveCategories(categoryItems);
            const catLinks = categoryIds.map((catId) => ({ manhwaId: id, categoryId: catId }));
            if (catLinks.length > 0) {
                await db.insert(manhwasToCategories).values(catLinks);
            }
        }
        ...
        if (tagItems && Array.isArray(tagItems)) {
            ...
            const tagIds = await resolveTags(tagItems);
            const tagLinks = tagIds.map((tId) => ({ manhwaId: id, tagId: tId }));
            if (tagLinks.length > 0) {
                await db.insert(manhwasToTags).values(tagLinks);
            }
        }
...
app.delete('/api/admin/manhwas/:id', checkRole(['admin']), async (c) => {
    try {
        ...
        for (const chapter of chaptersToDelete) {
            try {
                const pages = JSON.parse(chapter.content || '[]');
...
app.post('/api/admin/tags', checkRole(['admin', 'moderator']), async (c) => {
    try {
        ...
        if (!name || !slug)
            return c.json({ error: 'Нэр болон slug шаардлагатай' }, 400);
...
app.get('/api/admin/users', checkRole(['admin', 'moderator']), async (c) => {
    try {
        const allUsers = await db.select({
            ...
            bookmarksCount: sql `count(distinct ${bookmarks.id})`.mapWith(Number),
            historyCount: sql `count(distinct ${readingHistory.id})`.mapWith(Number),
        })
            .from(users)
...
app.get('/api/chapters/:id', async (c) => {
    try {
        ...
        if (!result[0])
            return c.json({ error: 'Бүлэг олдсонгүй' }, 404);
...
app.put('/api/admin/chapters/:id', checkRole(['admin', 'moderator']), async (c) => {
    try {
        ...
        if (existingChapter[0]) {
            try {
                const oldPages = JSON.parse(existingChapter[0].content || '[]');
                const newPages = JSON.parse(data.content || '[]');
...
app.delete('/api/admin/chapters/:id', checkRole(['admin']), async (c) => {
    try {
        ...
        if (chapterToDelete[0]) {
            ...
            try {
                const pages = JSON.parse(chapterToDelete[0].content || '[]');
                for (const pageUrl of pages) {
                    await deleteFile(pageUrl);
                }
            }
...
app.post('/api/admin/upload', checkRole(['admin', 'moderator']), async (c) => {
    try {
        ...
        const storageType = body['storage_type'] || undefined;
        if (!file) {
